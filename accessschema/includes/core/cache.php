...
/**
 * Initialize the cache system for the Access Schema plugin.
 *
...
function accessSchema_init_cache() {
	...
	/** This action is documented in includes/core/user-roles.php */
...
function accessSchema_get_all_roles( $force_refresh = false ) {
	...
	$roles = accessSchema_db_operation(
...
function accessSchema_build_role_hierarchy( $flat_roles ) {
	...
	foreach ( $flat_roles as $role ) {
		...
		} elseif ( isset( $role_map[ $role['parent_id'] ] ) ) {
...
function accessSchema_clear_all_caches() {
	...
	if ( $cache_table === $wpdb->get_var( "SHOW TABLES LIKE '{$cache_table}'" ) ) {
