...
function accessSchema_get_log_level() {
	...
	/**
	 * Filters the current log level for the Access Schema plugin.
	 *
...
function accessSchema_log_event( $user_id, $action, $role_path, $context = null, $performed_by = null, $level = 'INFO' ) {
	...
	$context_data['request_uri'] = isset( $_SERVER['REQUEST_URI'] ) ? sanitize_text_field( wp_unslash( $_SERVER['REQUEST_URI'] ) ) : '';
	$context_data['http_method'] = isset( $_SERVER['REQUEST_METHOD'] ) ? sanitize_text_field( wp_unslash( $_SERVER['REQUEST_METHOD'] ) ) : '';
	...
	$result = accessSchema_db_operation(
		function () use ( $wpdb, $table_name, $user_id, $action, $role_path, $context_data, $performed_by, $ip_address, $user_agent ) {
			return $wpdb->insert(
				$table_name,
				array(
					...
					'performed_by' => absint( $performed_by ? $performed_by : get_current_user_id() ),
...
function accessSchema_log_batch( $events ) {
	...
	foreach ( $events as $event ) {
		...
		$values[] = isset( $_SERVER['HTTP_USER_AGENT'] ) ? sanitize_text_field( wp_unslash( $_SERVER['HTTP_USER_AGENT'] ) ) : '';
	...
	return false !== $wpdb->query( $wpdb->prepare( $query, $values ) );
}
...
function accessSchema_get_user_logs( $user_id, $limit = 50 ) {
	...
	foreach ( $logs as &$log ) {
		$log['context'] = json_decode( $log['context'], true ) ? json_decode( $log['context'], true ) : array();
	}
