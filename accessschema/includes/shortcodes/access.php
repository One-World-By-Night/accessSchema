...
/**
 * Handle the [access_schema] shortcode for content access control.
 *
 * Conditionally displays content based on the current user's roles. Supports
 ...
 * @return string The rendered content, fallback content, or empty string.
 */
function accessSchema_shortcode_access( $atts, $content = null ) {
	// Check login state
	if ( ! is_user_logged_in() ) {
		/**
		 * Filters the output shown to users who are not logged in.
		 *
		 ...
		 * @param string|null $content The enclosed shortcode content.
		 */
	...
	$debug     = filter_var( $atts['debug'], FILTER_VALIDATE_BOOLEAN )
		&& defined( 'ACCESSSCHEMA_DEBUG' ) && ACCESSSCHEMA_DEBUG
	...
	// Check ALL condition
	if ( false !== $has_access && ! empty( $atts['all'] ) ) {
	...
	// Check ANY condition
	if ( false !== $has_access && ! empty( $atts['any'] ) ) {
	...
	// Check single role
	if ( false !== $has_access && ! empty( $atts['role'] ) ) {
		$role = trim( $atts['role'] );
