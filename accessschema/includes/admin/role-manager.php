...
function accessSchema_render_registered_roles_table() {
	...
	$per_page     = 25;
	$current_page = isset( $_GET['paged'] ) ? max( 1, absint( $_GET['paged'] ) ) : 1; // phpcs:ignore WordPress.Security.NonceVerification.Recommended -- Pagination parameter.
	$offset       = ( $current_page - 1 ) * $per_page;
	$search       = isset( $_GET['s'] ) ? sanitize_text_field( wp_unslash( $_GET['s'] ) ) : ''; // phpcs:ignore WordPress.Security.NonceVerification.Recommended -- Search parameter.
	...
	jQuery(document).ready(function($) {
		...
		$('#accessSchema-edit-form').on('submit', function(e) {
			...
			$btn.prop('disabled', true).text(accessSchema_ajax.i18n.saving);

			$.post(accessSchema_ajax.ajax_url, {
				action: 'accessSchema_save_role',
				...
				if (response.success) {
					...
					var $row = $('input[name="role_ids[]"][value="' + roleId + '"]').closest('tr');
					$row.find('td:nth-child(2)').text(response.data.name);
					$row.find('.accessSchema-edit-role').data('name', response.data.name);
					closeEditModal();
				} else {
					alert(response.data.message || accessSchema_ajax.i18n.error);
				}
			}).fail(function() {
				$btn.prop('disabled', false).text(origText);
				alert(accessSchema_ajax.i18n.error);
			});
		});
	});
...
function accessSchema_handle_role_actions() {
	...
	if ( isset( $_POST['submit'] ) && isset( $_POST['accessSchema_add_role_nonce'] ) &&
		...
		$paths = sanitize_textarea_field( wp_unslash( $_POST['full_paths'] ?? '' ) );
		...
		foreach ( $lines as $line_num => $path ) {
			...
			if ( $has_empty || empty( $segments ) ) {
				++$failed;
				$invalid_lines[] = $line_num + 1;
				continue;
			}
		...
		if ( ! empty( $invalid_lines ) ) {
			$redirect_args['invalid_lines'] = implode( ',', array_slice( $invalid_lines, 0, 10 ) );
		}
	...
	// Delete single role
	if ( isset( $_GET['action'] ) && 'delete' === sanitize_text_field( wp_unslash( $_GET['action'] ) ) && isset( $_GET['role_id'] ) ) {
		$role_id = absint( $_GET['role_id'] );

		if ( ! wp_verify_nonce( sanitize_text_field( wp_unslash( $_GET['_wpnonce'] ?? '' ) ), 'delete_role_' . $role_id ) ) {
			return;
		}
	...
	// Bulk actions
	if ( isset( $_POST['action'] ) && '-1' !== sanitize_text_field( wp_unslash( $_POST['action'] ) ) && isset( $_POST['role_ids'] ) ) {
		if ( ! wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST['_wpnonce'] ?? '' ) ), 'bulk-roles' ) ) {
			return;
		}
		...
		$role_ids = array_map( 'absint', wp_unslash( $_POST['role_ids'] ) );
...
function accessSchema_export_roles( $role_ids ) {
	...
	if ( false === $output ) {
		error_log( '[AccessSchema] Failed to open php://output for CSV export.' );
		wp_die( esc_html__( 'CSV export failed. Please try again.', 'accessschema' ) );
	}
